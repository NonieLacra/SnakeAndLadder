...
class FourPlayerGame : Fragment() {
    ...
    override fun onCreateView(
        ...
        val view = inflater.inflate(R.layout.fragment_game4players, container, false)
        ...
        val player4Image = view.findViewById<ImageView>(R.id.player4)
        ...
        val player4Text = view.findViewById<TextView>(R.id.player4Display)
        ...
        val player4Position = view.findViewById<TextView>(R.id.player4PosDisplay)
        ...
        player4Name = sharedPreferences.getString("Player 4", "")
        ...
        rollBtn.setOnClickListener {
            ...
            {
                val turnString = "$player4Name turns"
                turnText.text = turnString
            ...
            else
            {
                ...
                player4Pos += diceRoll
                player4Score += diceRoll

                if(player4Score >= 100){
                    winnerDialog(player4Name.toString())

                    val sharedPreferences = requireActivity().getSharedPreferences("winners", Context.MODE_PRIVATE)
                    val editor = sharedPreferences.edit()
                    val winnersJson = sharedPreferences.getString("winners", "[]") ?: "[]"
                    val winnersList = Gson().fromJson(winnersJson, object : TypeToken<MutableList<String>>() {}.type) as MutableList<String>
                    winnersList.add(player4Name.toString())
                ...
                for (i in snakeHead.indices) {
                    if (player4Pos == snakeHead[i]) {
                        player4Pos = snakeTail[i]
                        player4Score = snakeTail[i]
                    }
                }

                for (i in ladderBottom.indices) {
                    if (player4Pos == ladderBottom[i]) {
                        player4Pos = ladderTop[i]
                        player4Score = ladderTop[i]
                    }
                }

                val rollString = "$player4Name rolled a $diceRoll. You are now in tile $player4Pos"
                rollText.text = rollString

                val player4ScoreString = "$player4Score"
                player4Position.text = player4ScoreString
                ...
                boardImage.post {
                    ...
                    val direction = if ((player4Pos - 1) / 10 % 2 == 0) {
                        1
                    } else {
                        -1
                    }
                    ...
                    val xPosition = if (direction == 1) {
                        ((player4Pos - 1) % boardSize) * boardWidth / boardSize
                    } else {
                        (boardSize - (player4Pos - 1) % boardSize - 1) * boardWidth / boardSize
                    }

                    val yPosition =
                        ((boardSize + 2) - (player4Pos - 1) / boardSize) * boardHeight / boardSize

                    player4Image.x = xPosition.toFloat()
                    player4Image.y = yPosition.toFloat()
